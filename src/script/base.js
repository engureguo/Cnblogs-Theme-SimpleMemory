...
function Base() {
    ...
    this.rightMenuMous = function(parentObjectStr, subObjectStr) {
        ...
        parentObject.find('i').on({
            mouseover: function() {
                !$(this).hasClass('icon-zhiding') && $(this).rotate({
                    animateTo: -60,
                    duration: 250,
                    callback: function() {
                        $(this).rotate({
    ...
    this.homeInit = function() {
        ...
        // 设置主页图片
        let homeTopImg = window.cnblogsConfig.homeTopImg,
            bgImg;
        ...
        function prettifyCode() {
            ...
            switch (hltheme) {
                ...
                case 'desert':
                    ...
                    break;
                case 'sunburst':
                    require(['codeSunburst']);
                    break;
                case 'obsidian':
                    require(['codeObsidian']);
                    break;
                case 'doxy':
                    require(['codeDoxy']);
                    break;
        ...
        function setCodeBefore(type) {
            ...
            $.each(pre, function(i) {
                ...
                $.each(codeLine, function(j) {
                    ...
                    switch (type) {
                        case 1:
                            html += tools.HTMLEncode(codeLine[j]) + fg;
                            break;
                ...
                if ($('.post pre span').length > 0) {
                    ...
                    switch (hltype) {
                        case 'highlightjs':
                            ...
                            $('.mCSB_dragger_bar').css('background-color', color);
                            break;
                        case 'prettify':
                            color = $('.com').css('color');
                            $('.mCSB_dragger_bar').css('background-color', color);
                ...
                $.each(feedbackItem, function(i) {
                    ...
                    if (idInfo && idInfo.length > 0) {
                        ...
                        let op = $('#comment_' + id + '_avatar'),
                            patch = op.length > 0 ? $.trim(op.text()) :
